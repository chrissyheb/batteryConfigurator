...
function BatteryInverterCard({cfg, biIndex, setCfg, errorIndex}: any) {
  const biList = (cfg.Units.Main.Equipment.filter((e:any)=>e.Type==='BatteryInverter'))
  ...
  return (<div className="card"><div className="row" style={{justifyContent:'space-between'}}><span className="badge">BatteryInverter</span><button className="ghost" onClick={remove}>Entfernen</button></div>
    ...
    <div className="grid-2">
      <div className="card"><h3>Inverter</h3>
      ...
      <div className="card"><h3>Battery</h3>
    ...
    <div className="card"><h3>Modbus</h3>
      <SelectField leftIsType options={['(kein)', ...getModbusTypes()]} value={bi.Modbus ? 'TerraModbus' : '(kein)'} onChange={(v:string)=>{
        const c=structuredClone(cfg); if (v==='(kein)') delete c.Units.Main.Equipment[listIndex].Modbus;
        else c.Units.Main.Equipment[listIndex].Modbus = { Name:'', Type:'TerraModbus', Guid: bi.Modbus?.Guid ?? '' }; setCfg(c)
      }} error={errorAt(errorIndex, ['Units','Main','Equipment', listIndex, 'Modbus'])} />
      {bi.Modbus && (<><TextField leftLabel="Name" value={bi.Modbus.Name} onChange={(v:string)=>{ const c=structuredClone(cfg); c.Units.Main.Equipment[listIndex].Modbus.Name=v; setCfg(c) }} error={errorAt(errorIndex, ['Units','Main','Equipment', listIndex, 'Modbus','Name'])} />
...
export default function MainForm({cfg, setCfg, errorIndex}: {cfg:any, setCfg:(c:any)=>void, errorIndex:any}) {
  const addSmartMain = () => { const has = cfg.Units.Main.Equipment.some((e:any)=>e.Type==='SmartmeterMain'); if (has) return
    const sm = { Name:'Smartmeter', Displayname:'', Type:'SmartmeterMain', Hardware:'Virtual', Guid: uuid() }
    const c = structuredClone(cfg); c.Units.Main.Equipment.unshift(sm); setCfg(c) }
  const addBatteryInv = () => { const bi = {
    ...
    const c = structuredClone(cfg); c.Units.Main.Equipment.push(bi); setCfg(c) }
  ...
  return (<div className="card stack"><h2>Main</h2>
    <div className="field"><label>Type</label>
      <select value={cfg.Units.Main.Type} onChange={(e)=>setCfg({ ...cfg, Units: { ...cfg.Units, Main: { ...cfg.Units.Main, Type: e.target.value } } })}>
        <option>Blokk</option><option>Terra</option></select>
      <div className="inline-error">{errorAt(errorIndex, ['Units','Main','Type'])}</div></div>
    <div className="row"><button onClick={addSmartMain}>+ SmartmeterMain</button><button onClick={addBatteryInv}>+ BatteryInverter</button></div>
    {smMain && (<div className="card"><span className="badge">SmartmeterMain</span>
    ...
    {(cfg.Units.Main.Equipment.filter((e:any)=>e.Type==='BatteryInverter')).map((_:any, idx:number)=>(
