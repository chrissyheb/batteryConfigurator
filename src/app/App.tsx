...
export default function App() {
  ...
  return (<div className="container"><header className="row" style={{justifyContent:'space-between'}}>
    <h1>Battery Configurator</h1><div className="row"><button onClick={onExport}>Export</button>
    <label className="btn">Import<input hidden type="file" accept="application/json" onChange={(e)=>{ const f=e.target.files?.[0]; if (f) onImport(f) }} /></label></div></header>
    {!isValid && (<div className="error-panel"><strong>Fehler</strong><ul>{issues.map((er, i) => (<li key={i}>{er.message}</li>))}</ul></div>)}
    <section className="card"><h2>Global</h2>
      <div className="field"><label>Type</label><select value="Global" onChange={()=>{}}><option>Global</option></select><span /></div>
      ...
      <div className="field"><label>Hardwarevariante</label><select value={state.ModularPlc.Hardwarevariante} onChange={(e)=>setCfg({ ...state, ModularPlc: { ...state.ModularPlc, Hardwarevariante: e.target.value } })}><option>BlokkV3</option><option>Terra</option><option>Variante1</option></select><small className="hint">Wechsel ändert nichts automatisch – nur Validierungsfehler.</small></div>
    </section>
